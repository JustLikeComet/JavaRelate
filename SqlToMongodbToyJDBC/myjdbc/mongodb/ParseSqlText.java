...
public class ParseSqlText {
	...
	public static void parseSelect(PlainSelect ps) {
		
		final List<SelectItem> list = ps.getSelectItems();
		...
		Table fromItem = (Table) ps.getFromItem();
		System.out.println(fromItem.getName());
		
		List<Join> joinList = ps.getJoins();
		if (joinList != null && joinList.size() > 0) {
			joinList.get(0).getRightItem();
		}
		
		Expression where = ps.getWhere();
		if(where!=null){
			Document mongoWhereCond = parseWhereStament(where);
		}
	}
	
	public static void execSqlInMongoDB(String sql) throws JSQLParserException {
	...
	public static void main(String[] args) throws Exception {
		//execSqlInMongoDB("SELECT * FROM mytable WHERE mytable.col = 9");
		execSqlInMongoDB("SELECT * FROM test01,test02");
	}
